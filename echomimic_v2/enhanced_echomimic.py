    ...
    for attempt in range(max_retries + 1):
        try:
            with gpu_memory_manager():
                ...
                test_img = Image.open(ref_image_path)
                print(f"   ✅ 图像尺寸: {test_img.size}, 模式: {test_img.mode}")
                
                import librosa
                test_audio, test_sr = librosa.load(audio_path, sr=None)
                print(f"   ✅ 音频长度: {len(test_audio)/test_sr:.2f}s, 采样率: {test_sr}Hz")
                
                pose_files = list(Path(pose_dir).glob("*"))
                print(f"   ✅ 姿势文件数: {len(pose_files)}")
                
                # 执行推理
                with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
                    print(f"\n🚀 开始推理，确认参数:")
                    print(f"   reference_image={ref_image_path}")
